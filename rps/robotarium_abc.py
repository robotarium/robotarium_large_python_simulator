...
class RobotariumABC(ABC):
    def __init__(
        ...
        boundaries=[-10, -10, 20, 20],
    ):

        # Check user input types
        assert isinstance(number_of_robots, int), (
            "The number of robots used argument (number_of_robots) provided to create the Robotarium object must be an integer type. Recieved type %r."
            % type(number_of_robots).__name__
        )
        assert isinstance(initial_conditions, np.ndarray), (
            "The initial conditions array argument (initial_conditions) provided to create the Robotarium object must be a numpy ndarray. Recieved type %r."
            % type(initial_conditions).__name__
        )
        assert isinstance(show_figure, bool), (
            "The display figure window argument (show_figure) provided to create the Robotarium object must be boolean type. Recieved type %r."
            % type(show_figure).__name__
        )
        assert isinstance(sim_in_real_time, bool), (
            "The simulation running at 0.033s per loop (sim_real_time) provided to create the Robotarium object must be boolean type. Recieved type %r."
            % type(show_figure).__name__
        )

        # Check user input ranges/sizes
        assert number_of_robots >= 0 and number_of_robots <= 50, (
            "Requested %r robots to be used when creating the Robotarium object. The deployed number of robots must be between 0 and 50."
            % number_of_robots
        )
        if initial_conditions.size > 0:
            assert initial_conditions.shape == (3, number_of_robots), (
                "Initial conditions provided when creating the Robotarium object must of size 3xN, where N is the number of robots used. Expected a 3 x %r array but recieved a %r x %r array."
                % (
                    number_of_robots,
        ...
        if self.show_figure:
            ...
            for i in range(number_of_robots):
                ...
                p = patches.Rectangle(
                    (
                        ...
                        * np.array(
                            (
                                np.cos(self.poses[2, i] + math.pi / 2),
                                np.sin(self.poses[2, i] + math.pi / 2),
                            )
                        )
                        + 0.04
                        * np.array(
                    ...
                    self.robot_length,
                    self.robot_width,
                    (self.poses[2, i] + math.pi / 4) * 180 / math.pi,
                    facecolor="#FFD700",
                    edgecolor="k",
                )
            ...
            self.axes.set_xlim(
                self.boundaries[0] - 0.1, self.boundaries[0] + self.boundaries[2] + 0.1
            )
            self.axes.set_ylim(
                self.boundaries[1] - 0.1, self.boundaries[1] + self.boundaries[3] + 0.1
            )
            ...
            plt.subplots_adjust(
                left=-0.03, right=1.03, bottom=-0.03, top=1.03, wspace=0, hspace=0
            )
    ...
    def _diff_to_uni(self, dxdd):
        ...
        l = self.base_length
        dxu = np.vstack(
            (r / (2) * (dxdd[0, :] + dxdd[1, :]), r / l * (dxdd[1, :] - dxdd[0, :]))
        )
    ...
    def _validate(self, errors={}):
        ...
        for i in range(N):
            ...
            if x < b[0] or x > (b[0] + b[2]) or y < b[1] or y > (b[1] + b[3]):
                if "boundary" in errors:
                    ...
                    errors[
                        "boundary_string"
                    ] = "iteration(s) robots were outside the boundaries."

        for j in range(N - 1):
            for k in range(j + 1, N):
                first_position = p[:2, j] + self.collision_offset * np.array(
                    [np.cos(p[2, j]), np.sin(p[2, j])]
                )
                second_position = p[:2, k] + self.collision_offset * np.array(
                    [np.cos(p[2, k]), np.sin(p[2, k])]
                )
                if np.linalg.norm(first_position - second_position) <= (
                    ...
                    if "collision" in errors:
                        ...
                        errors[
                            "collision_string"
                        ] = "iteration(s) where robots collided."
        ...
        if np.any(exceeding):
            if "actuator" in errors:
                ...
                errors[
                    "actuator_string"
                ] = "iteration(s) where the actuator limits were exceeded."
