...
class Robotarium(RobotariumABC):
    ...
    def get_poses(self):
        ...
        assert (
            not self._checked_poses_already
        ), "Can only call get_poses() once per call of step()."
    ...
    def call_at_scripts_end(self):
        ...
        print("##### DEBUG OUTPUT #####")
        print(
            "Your simulation will take approximately {0} real seconds when deployed on the Robotarium. \n".format(
        ...
        if bool(self._errors):
            if "boundary" in self._errors:
                ...
                print(
                    "\t Simulation had {0} {1}\n".format(
                        boundary_violations, self._errors["boundary_string"]
                    )
                )
            if "collision" in self._errors:
                ...
                print(
                    "\t Simulation had {0} {1}\n".format(
                        collision_violations, self._errors["collision_string"]
                    )
                )
            if "actuator" in self._errors:
                print(
                    "\t Simulation had {0} {1}".format(
                        self._errors["actuator"], self._errors["actuator_string"]
                    )
                )
        else:
            print(
    ...
    def step(self):
        ...
        self.poses[0, :] = (
            self.poses[0, :]
            + self.time_step * np.cos(self.poses[2, :]) * self.velocities[0, :]
        )
        self.poses[1, :] = (
            self.poses[1, :]
            + self.time_step * np.sin(self.poses[2, :]) * self.velocities[0, :]
        )
        ...
        if self.show_figure:
            ...
            for i in range(self.number_of_robots):
                ...
                self.chassis_patches[i].angle = (
                    (self.poses[2, i] - math.pi / 2) * 180 / math.pi
                )

                self.right_wheel_patches[i].center = (
                    self.poses[:2, i]
                    + self.robot_length
                    / 2
                    * np.array(
                ...
                self.left_wheel_patches[i].center = (
                    self.poses[:2, i]
                    + self.robot_length
                    / 2
                    * np.array(
                ...
                self.right_led_patches[i].center = (
                    self.poses[:2, i]
                    + 0.75
                    * self.robot_length
                    / 2
                    * np.array((np.cos(self.poses[2, i]), np.sin(self.poses[2, i])))
                    - 0.04
                    * np.array((-np.sin(self.poses[2, i]), np.cos(self.poses[2, i])))
                    + self.robot_length
                    / 2
                    * np.array((np.cos(self.poses[2, i]), np.sin(self.poses[2, i])))
                )
                self.left_led_patches[i].center = (
                    self.poses[:2, i]
                    + 0.75
                    * self.robot_length
                    / 2
